...
```{r}
read_excel("./data/5Ws_2023_v2/Copy of eie_5ws_template_v2_childfund.xlsx", 
           sheet = 2, skip = 2) %>% 
  setNames(v2_names_5ws) %>% 
  
  filter(!is.na(implementing_partner))
```


```{r}
read_excel("./data/Copy of eie_5ws_template_v2_childfund.xlsx", 
             sheet = "5W_Data_Entry", 
             skip = 2, 
             col_types = c(
               "text", "text", "text", "text", "text", "text", 
...
eie %>% 
  ...
  write_csv(paste0("./data/eie_5ws_ocha", 
...
eie %>% 
  ...
  summarise(reached = sum(total_reached, na.rm = TRUE), 
            ...
            .groups = "drop") %>%
  write_csv(paste0("./data/ocha_reached_by_indicator", 
                  format(Sys.time(), '%Y%m%d'), 
                  ".csv"))
