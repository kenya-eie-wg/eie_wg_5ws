...
eie <- read_csv("./data/eie_5ws.csv") %>% 
  ...
  mutate(total_reached = case_when(
    ...
    str_detect(indicator, "children and youth benefiting from resilience enhancing programmes") ~ boys + girls, 
                                   TRUE ~ total_reached)) %>% 
  filter(month %out% c("December"))
```



[Previous months' reports](https://kenya-eie-wg.github.io/) 
...
```{r}
eie %>% 
  filter(str_detect(indicator_short, "6")) %>% 
  pc_plot() +
...
```{r}
summary_table %>% 
  filter(str_detect(Indicator, "7")) %>% 
  flextable() %>% 
  set_caption(paste0("EiE Indicator 7, as of ", params$report_date)) %>% 
  set_table_properties(layout = "autofit", width = .99) 
```
...
```{r}
indicator_map(eie, "7", county_target) +
  labs(title = "Targets", 
       fill = "Target")  + 
indicator_map(eie, "7", beneficiaries) + 
  scale_fill_viridis(limits = range(targets_range$indicator6), 
                     labels = comma, 
                     na.value = "grey80") + 
  labs(title = "Total reached", 
       fill = "Reached") +
  plot_annotation(title = "7. Children benefited from resilience enhancing programmes, target and reached", 
                  subtitle = paste0("As of ", params$report_date)) 
...
In `r params$report_month` 2023, `r eie %>% filter(month %in% params$report_month & str_detect(indicator_short, "7") & activity_status == "Completed") %>% {sum(.$total_reached, na.rm = TRUE)} %>% format(big.mark = ",")` persons were reached by interventions contributing to this indicator, of which `r eie %>% filter(month %in% params$report_month & str_detect(indicator_short, "7") & activity_status == "Completed") %>% {sum(.$boys, na.rm = TRUE)} %>% format(big.mark = ",")` were boys and `r eie %>% filter(month %in% params$report_month & str_detect(indicator_short, "7") & activity_status == "Completed") %>% {sum(.$girls, na.rm = TRUE)} %>% format(big.mark = ",")` and were girls. 
...
partner_map <- eie %>% 
  ...
  geom_sf_text(aes(label = paste0( 
    county, "\n", partners), 
                ...
                "Female_reached%: ", female_pc, "%, ", "Male_reached%: ", male_pc, "%", 
                "\n",  
                acronym)), 
  ...
  labs(title = "Education in Emergencies Working Group operational presence", 
       subtitle = paste0("as of ", params$report_date), 
       fill = "Partners")
...
IPC data extracted from [here](https://www.ipcinfo.org/ipc-country-analysis/details-map/en/c/1156541/?iso3=KEN). 
