...
combined %>% 
  ...
  filter(sub_indicator == "indicator1_all_total_children")
```
...
UNICEF also supported the training of `r bom_indicator %>% filter(sex_modifier == "total") %>% pull(value)` (`r bom_indicator %>% filter(sex_modifier == "male") %>% pull(value)` men and `r bom_indicator %>% filter(sex_modifier == "female") %>% pull(value)` women) members of the Enrolment Drive Committees (EDCs) and school Board of Management (BoM) in Baringo and Turkana counties on life skills, child safeguarding and their roles/responsibilities, child protection and referral mechanisms. 
...
[UNICEF monthly sitrep combined](https://github.com/kenya-eie-wg/eie_wg_5ws/blob/main/data/combined.csv)
