---
title: "Kenya EIE Working Group -- UNICEF August sitrep inputs"
date:  "28 August 2023"
output: 
  word_document:
    toc: no
...
previous_months <- c("January", "February", "March", "April", "May", "July", "August")
...
indicator5 <- eie %>% 
  replace_na(list(boys = 0, girls = 0, men = 0, women = 0)) %>% 
  # Total reached of indicator 1 is only boys and girls 
  mutate(total_reached = ifelse(str_detect(indicator, "5"), 
                                men + women, 
                                total_reached)) %>% 
...
bom_indicator <- combined %>% 
  ...
  mutate(value = ifelse(sex_modifier == "total", 
                        sum(value, na.rm = TRUE)[sex_modifier %in% c("male", "female")], 
                        value))
...
UNICEF also supported the training of `r bom_indicator %>% filter(sex_modifier == "total") %>% pull(value)` (`r bom_indicator %>% filter(sex_modifier == "male") %>% pull(value)` men and `r bom_indicator %>% filter(sex_modifier == "female") %>% pull(value)` women) members of the Enrolment Drive Committees (EDCs) and school Board of Management (BoM) in `r bom_indicator_counties` county on life skills, child safeguarding and their roles/responsibilities, child protection and referral mechanisms. 
