...
```{r}
read_excel("./data/unicef_internal/Copy of Monthly Education SitRep_Jul23_lzo.xlsx") %>%
setNames(messy) %>% 
    slice(10:248) %>% 
    ...
    filter(!is.na(month)) %>% 
    select(-contains("x")) %>%
  mutate_at(vars(-matches("narrative|month")), ~as.numeric(.)) %>% 
  mutate_at(vars(-matches("narrative|month")), ~ ifelse(is.na(.), 0, .)) %>% 
  mutate(indicator1_all_girls = indicator1_host_community_girls + indicator1_idps_girls + indicator1_refugees_girls, 
         indicator1_all_boys = indicator1_host_community_boys + indicator1_idps_boys + indicator1_refugees_boys, 
         indicator1_all_total_children = indicator1_all_boys + indicator1_all_girls, 
         indicator2_all_boys = indicator2_host_community_boys + indicator2_idps_boys + indicator2_refugees_boys, 
         indicator2_all_girls = indicator2_host_community_girls + indicator2_idps_girls + indicator2_refugees_girls,
         indicator2_all_total_children = indicator2_all_boys + indicator2_all_girls, 
         indicator3_all_boys = indicator3_host_community_boys + indicator3_idps_boys + indicator3_refugees_boys,
         indicator3_all_girls = indicator3_host_community_boys + indicator3_idps_girls + indicator3_refugees_girls, 
         indicator3_all_total_children = indicator3_all_boys + indicator3_all_girls, 
         indicator4_all_female = indicator4_host_community_female + indicator4_idps_female + indicator4_refugees_female, 
         indicator4_all_male = indicator4_host_community_male + indicator4_idps_male + indicator4_refugees_male, 
         indicator4_all_total_adults = indicator4_all_female + indicator4_all_male, 
         indicator5_all_female = indicator5_host_community_female + indicator5_idps_female + indicator5_refugees_female, 
         indicator5_all_male = indicator5_host_community_male + indicator5_idps_male + indicator5_refugees_male, 
         indicator5_all_total_adults = indicator5_all_female + indicator5_all_male,
         indicator7_all_girls = indicator7_host_community_girls + indicator7_idps_girls + indicator1_refugees_girls, 
         indicator7_all_boys = indicator7_host_community_boys + indicator7_idps_boys + indicator7_refugees_boys, 
...
```{r}
trial <- read_excel("./data/unicef_internal/Copy of Monthly Education SitRep_Jul23_lzo.xlsx") %>% 
  setNames(messy) %>% 
    slice(10:248) %>% 
    mutate(county = word(month, 1)) %>%
    naniar::replace_with_na(list(county = "July", 
...
trial %>% 
  ...
  mutate(indicator1_host_community_total_children = indicator1_host_community_boys +
         ...
         indicator1_host_community_total_children = indicator1_host_community_boys +
           indicator1_host_community_girls, 
...
## Changes to combined, to ready for archiving

As the old sitrep template was deprecated, this whole section is being readied for archival. 

```{r}
combined <- combined %>%
  mutate(eie_indicator = case_when(str_detect(unicef_indicator, "1") ~ "indicator 1 # of children and youth accessing ECD spaces/schools in drought affected areas", 
                                   str_detect(unicef_indicator, "2") ~ "indicator 7 # of children benefitting from child-friendly environment (any construction or rehabilitation)", 
                                   str_detect(unicef_indicator, "3") ~ "indicator 3 # of learners and teachers reached with learning and teaching materials", 
                                   str_detect(unicef_indicator, "4") ~ "indicator 5.1 # of teachers trained on any programme, including resilience", 
                                   str_detect(unicef_indicator, "5") ~ "indicator 5.2 # of BoM trained on any programme, including resilience", 
                                   str_detect(unicef_indicator, "6") ~ "indicator 5.3 # of government officials trained on any programme, including resilience", 
                                   str_detect(unicef_indicator, "7") ~ "indicator 6 # of children and youth benefiting from resilience enhancing programmes (MHPSS, DRR & Life Skills, etc)")) 

combined %>% 
write_csv("./data/old_table_combined.csv")

old_table_eie_match <- combined %>% 
  filter(beneficiary_group != "all") %>% 
  filter(sex_modifier != "total") %>% 
  group_by(month, county, unicef_indicator, eie_indicator, beneficiary_group, sex_modifier, age_modifier) %>% 
  summarise(value = sum(value, na.rm = TRUE)) %>% 
  pivot_wider(names_from = c(sex_modifier, age_modifier), 
              values_from = value) %>% 
  replace_na(list(female_children = 0, male_children = 0, female_adults = 0, male_adults = 0)) %>% 
  mutate(beneficiaries = female_children + male_children + female_adults + male_adults) %>% 
  filter(beneficiaries > 0)

old_table_eie_match %>% 
  write_csv("./data/old_table_eie_match.csv")
...
combined %>% 
  ...
  unnest() %>% 
  select(-`NA`, -cwd) %>% 
  replace_na(list(all = 0, host_community = 0, idps = 0, refugees = 0)) %>% 
  mutate(check = ifelse(all != host_community + idps + refugees, FALSE, TRUE)) %>% 
  filter(check == FALSE & !str_detect(unicef_indicator, "6"))
...
data_entry_wide %>% 
  write_xlsx("./data/data_entry.xlsx", 
             sheeName = "data_entry")
