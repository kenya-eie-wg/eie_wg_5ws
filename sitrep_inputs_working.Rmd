...
```{r}
combined %>% 
mutate(age_modifier = case_when(str_detect(sub_indicator, "1|3|7") & is.na(age_modifier) ~ "children", 
                                str_detect(sub_indicator, "4|5|6") & is.na(age_modifier) ~ "adults",
                                str_detect(sub_indicator, "2") & !is.na(sex_modifier) ~ "children", 
                                TRUE ~ age_modifier)), 
       beneficiary_group = case_when(str_detect(sub_indicator, "cwd") & !is.na(beneficiary_group) ~ "cwd", 
                                     TRUE ~ beneficiary_group)) 
combined %>%   
  distinct(unicef_indicator, 
           sub_indicator,
           sex_modifier, 
           age_modifier, 
           beneficiary_group) 
...
crossing(combined %>% distinct(county),
  ...
  mutate_at(vars(!matches("narrative|county|month")), 
            ~ as.numeric(.)) %>% 
  mutate_at(vars(matches("all_total_children")), ~ ifelse(sum))
  mutate(indicator1_all_total_children = ifelse(indicator1_all_total_children))
  glimpse()
```
